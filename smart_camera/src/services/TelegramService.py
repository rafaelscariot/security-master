...
class TelegramService:
    ...
    def send_notification(self, status_detection, userId):
        try:
            devices = self.devices(f'{self.BASE_HOST}/device/surveillance/{userId}')
            ...
            if devices == []:
                ...
                for device in devices:
                    ...
                    img_request = 'curl -s -X POST https://api.telegram.org/bot' + self.token + \
                        '/sendPhoto -F chat_id=' + \
                        device["chatId"] + " -F photo=@" + image_path
                ...
                SaveAlertService().save(
                    userId, status_detection['type'], status_detection['region'])
